#!/bin/bash
config.sh L1D_num_ways_08_config.json
make

config.sh L1D_num_ways_16_config.json
make

config.sh L1D_num_ways_28_config.json
make

config.sh L1D_num_ways_29_config.json
make

config.sh L1D_num_ways_32_config.json
make

config.sh L1D_num_ways_50_config.json
make

config.sh L1D_num_ways_64_config.json
make

config.sh L1D_num_ways_78_config.json
make

config.sh L1D_num_ways_79_config.json
make

config.sh L1D_num_ways_128_config.json
make
